...
function removeNumbering() {
    ...
    headings.forEach(heading => {
        ...
        const selfLink = heading.nextElementSibling?.tagName === "A"
...
function fetchChangelogForPeriod(startDate, endDate, elementId) {
  ...
  return fetch(url)
    ...
    .then(data => {
      ...
      const filteredPRs = mergedPRs
        .filter(pr => !/^Merge pull|^tidy|^TRAVIS|^ADMIN|^Generated by|respec|(editorial)|.md|editorial|Editiorial|edtiorial|chore|^refactor|^tests?|^docs|w3cgruntbot|index.html|typo|^nit/i.test(pr.title))
